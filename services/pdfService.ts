...
export class PDFProcessor {
  ...
  async extractTextSummary(): Promise<{ pageCount: number; pageSummaries: string[] }> {
   ...
   */
  async detectChapters(granularity: Granularity): Promise<Chapter[]> {
    const arrayBuffer = await this.file.arrayBuffer();
    const loadingTask = getDocument({ data: arrayBuffer });
    const pdf = await loadingTask.promise;
    const numPages = pdf.numPages;
    ...
    if (outline && outline.length > 0) {
      const chaptersFromBookmarks = await this.extractChaptersFromOutline(pdf, outline, granularity);
      if (chaptersFromBookmarks.length > 0) {
        return this.normalizeChapters(chaptersFromBookmarks, numPages);
      }
    }

    // 2. Fallback to heuristic rules (Track B)
    return await this.detectChaptersByRules(pdf, granularity);
  }

  /**
   * Converts PDF outline hierarchical structure into flat Chapter array based on granularity.
   */
  private async extractChaptersFromOutline(pdf: any, outline: any[], granularity: Granularity): Promise<Chapter[]> {
    const chapters: Chapter[] = [];

    const traverse = async (items: any[], currentLevel: number, parentIndex: string, parentTitle: string) => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (pageNumber > 0) {
          chapters.push({
            ...
            indexCode: this.padIndexCode(indexCode),
   ...
   */
  private padIndexCode(code: string): string {
   ...
   */
  private normalizeChapters(chapters: Chapter[], totalPages: number): Chapter[] {
   ...
   */
  private async detectChaptersByRules(pdf: any, granularity: Granularity): Promise<Chapter[]> {
    const numPages = pdf.numPages;
    ...
    for (let i = 1; i <= numPages; i++) {
      ...
      for (const item of topItems) {
        ...
        const isNearTop = item.transform[5] > 400;
    ...
    const chapters = candidates.map((c, idx) => ({
      ...
      startPage: c.page,
      endPage: c.page,
      indexCode: this.padIndexCode(String(idx + 1)),
    ...
    return this.normalizeChapters(chapters, numPages);
  }
  ...
  async splitPdfByChapters(
    ...
    const zip = new JSZip();
    const bookName = this.sanitizeFileName(this.file.name.replace(/\.pdf$/i, ''));
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      if (pageIndices.length > 0) {
        ...
        const p1 = this.pad(indexParts[0] || String(i + 1));
        const p2 = this.pad(indexParts[1] || '0');
        const p3 = this.pad(indexParts[2] || '0');
        ...
        const chapterName = this.sanitizeFileName(chapter.title);
